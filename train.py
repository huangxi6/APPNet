...
def get_arguments():
    ...
    """
    parser = argparse.ArgumentParser(description="PSN Network")
...
def main():
    ...
    f = open('./t_lip_appn_150_continue_2.txt','w')
    f.write(str(args)+'\n')
...
##############################################load data2
    """res = './s_lip_appn_150/lip_epoch_21.pth'
    #res = './s_glr0.007_updatedenaspp2/Person_epoch_29.pth'
...
##########################################
    ...
    #criterion = CriterionOhemDSN(thresh=0.7, min_kept=100000)
    ...
    trainloader = data.DataLoader(HumanDataSet(args.data_dir, args.dataset, crop_size=input_size,transform=transform),batch_size=args.batch_size * len(gpus), shuffle=True, num_workers=2, pin_memory=True)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for i_iter, batch in enumerate(trainloader):
            ...
            images, labels, label_seg1, meta = batch
            labels = labels.long().cuda(non_blocking=True)
            label_seg1 = label_seg1.long().cuda(non_blocking=True)

            preds = model(images)

            loss = criterion(preds, [labels, label_seg1])
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
